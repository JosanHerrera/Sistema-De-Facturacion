...
const ClientEdit = ({ setPage, clientId, setClients }) => {
  ...
  useEffect(() => {
    ...
    const clientToEdit = currentClients.find(cli => cli.id === clientId);
  ...
  const handleSaveClient = () => {
    ...
    const currentClients = getStorage('clients') || initialClients;
    const updatedClients = currentClients.map(cli => cli.id === updatedClient.id ? updatedClient : cli);
  ...
  return (
    <div className="p-6 max-w-md mx-auto">
      <h2 className="text-3xl font-bold mb-6 text-gray-800">Editar Cliente</h2>
